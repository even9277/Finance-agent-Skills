...
async def init_user(body: UserInitRequest, db: AsyncSession = Depends(get_db)):
    """
    新用户冷启动：
    - 若用户不存在则创建
    ...
    if body.preferences:
        await memory_service.cold_start(body.user_id, body.preferences, db)
