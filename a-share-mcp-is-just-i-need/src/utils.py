...
def baostock_login_context():
    """
    上下文管理器，确保当前进程内 Baostock 只登录一次，并在整个 MCP 进程生命周期内保持登录状态。

    设计目标：
    - 避免在高并发下重复 login/logout 导致的“用户未登录(10001001)”错误
    - 仍然允许多个工具并发调用 Baostock，只在首次使用时做一次登录
    - 抑制 Baostock 登录过程中的标准输出，保持终端整洁
    """
    global _baostock_logged_in

    # 只在第一次使用时登录一次，后续复用同一会话
    if not _baostock_logged_in:
        with _baostock_login_lock:
            ...
            if not _baostock_logged_in:
                ...
                lg = bs.login()
                logger.debug(
                    "Login result: code=%s, msg=%s", lg.error_code, lg.error_msg
                )
                ...
                logger.info("Baostock login successful (session initialized).")

    # 此处不再做登出，保持会话在整个进程生命周期内有效
    try:
        yield  # API 调用在这里进行
    finally:
        # 不执行 bs.logout()，避免在并发场景下把其他调用中的会话注销掉
        # 如果需要显式登出，应在进程退出前由上层统一处理
        pass
