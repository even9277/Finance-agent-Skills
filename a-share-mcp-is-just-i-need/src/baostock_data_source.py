...
class BaostockDataSource(FinancialDataSource):
    ...
    def _load_risk_model(self):
        """加载风险模型。若未部署本地模型（路径不存在），则返回 None，新闻爬虫仍可运行并显示「未配置」。"""
        try:
            import os
            risk_model_path = os.environ.get("RISK_MODEL_PATH", "/root/code/Finance/qwen_risk_model")
            base_model_name = os.environ.get("QWEN_BASE_MODEL_PATH", "/root/code/Finance/Qwen")
            if not os.path.exists(risk_model_path) or not os.path.exists(base_model_name):
                logger.info("未配置风险模型路径或路径不存在，跳过加载，新闻分析将不包含本地风险分数。")
                return None, None
    ...
    def _load_sentiment_model(self):
        """加载情感模型。若未部署本地模型（路径不存在），则返回 None，新闻爬虫仍可运行并显示「未配置」。"""
        try:
            import os
            sentiment_model_path = os.environ.get("SENTIMENT_MODEL_PATH", "/root/code/Finance/qwen_sentiment_model")
            base_model_name = os.environ.get("QWEN_BASE_MODEL_PATH", "/root/code/Finance/Qwen")
            if not os.path.exists(sentiment_model_path) or not os.path.exists(base_model_name):
                logger.info("未配置情感模型路径或路径不存在，跳过加载，新闻分析将不包含本地情感分数。")
                return None, None
            from transformers import AutoTokenizer, AutoModelForCausalLM
            from peft import PeftModel
            import torch
