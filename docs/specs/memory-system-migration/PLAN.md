...
- What changed: Milestone 2 added typed memory/state/governance/command/retrieval contracts, deterministic policy guards, eight Alembic-owned authority tables, a reversible additive revision, and a chat transaction seam that commits the user message, assistant message, initial Working State, and safe `TURN_COMMITTED` Outbox task together. Only Alembic was added as a direct dependency.
- What was verified: Upgrade/downgrade/re-upgrade and legacy row readability, core ORM/migration schema parity, transaction success/rollback/idempotency/same-session concurrency/tenant and cross-session isolation, Working State source authority, precise persistence-error semantics, fail-closed migration targeting and downgrade, regular backend-image packaging, changed-scope zero-error Ruff/Pyright, unchanged full-scan debt counts, controlled-mainline and root regressions, and a real isolated PostgreSQL/Nginx/FastAPI Compose turn with direct state/outbox row assertions. No paid/live model, Tushare, Mem0, Redis, pgvector, or production service/write was used.
- What remains risky: The Working State is currently initialized and propagated but not yet extracted or merged; Rolling Summary/compaction remains dormant; candidate governance, Redis acceleration, Mem0/pgvector retrieval, memory commands/UI, and final memory Trace stages remain intentionally unimplemented. Application-startup migration needs a dedicated migration job or advisory lock before multi-replica production. Existing 81 Ruff errors, 80 Pyright errors, deprecation warnings, and Windows xfail-reason rendering are unchanged tracked debt.
- What should be improved next: Execute Milestone 3 only to implement deterministic Working State transitions, token budgeting, durable compaction, versioned Rolling Summary, and controlled-context injection on top of the M2 authority/outbox foundation.
...
Milestones 0, 1, and 2 are complete. The first unchecked milestone is Milestone 3. Execute only Working State extraction/merge, token budgeting, Rolling Summary/compaction, and controlled-context integration; do not install Redis/Mem0/pgvector dependencies early, and stop before Milestone 4.
