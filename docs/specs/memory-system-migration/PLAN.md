...
- What changed: Milestone 3 made session memory active in the only public controlled-chat path. The workflow now emits typed active-entity/candidate/constraint/reply-preference updates; the repository persists field events with state-version CAS in the foreground transaction; context packing reserves output/safety/stage budgets and preserves a contiguous raw tail; committed turns can enqueue a frozen, idempotent summary task; and the unified Worker validates, retries/dead-letters/cancels stale work, writes versioned metadata/snapshots, and preserves last-good behavior. The legacy profile-from-summary helper was removed without adding a compatibility queue, Redis, Mem0, pgvector, or a new dependency.
- What was verified: 48 M3 focused tests covered set/merge/clear/expire/no-op/version/event rules, explicit entity switching and follow-up inheritance, new-topic segment expiry, current-instruction precedence, budget overflow/contiguous-tail behavior, summary schema/source/protected-tail validation, foreground transaction/outbox behavior, provider bootstrap, retry/dead-letter, stale CAS, expired-lease recovery, two-Worker fencing, persisted-task tamper rejection, and a five-case offline STM eval. Static, backend, Agent, eval, frontend, root, Compose-config, normal-image packaging, and isolated full-stack E2E gates passed. The final root run reported 191 passed; the rebuilt Compose run reported 114 passed. E2E used real PostgreSQL/Alembic/FastAPI/Nginx/Vue packaging with deterministic model/tool providers; no paid/live model, real Tushare, Mem0, Redis, pgvector, or production service/write was used.
- What remains risky: Redis hot-state/cache coordination, long-term candidate governance, Mem0/pgvector retrieval, user memory commands/UI, and final memory-specific Trace/evaluation/live-provider closure remain intentionally unimplemented. Summary generation now has bounded retry, last-good semantics, expired-lease recovery, and claim fencing; richer operational metrics remain a later hardening requirement. Existing deprecation warnings and legacy/vendor full-scan debt remain tracked outside M3.
- What should be improved next: Execute Milestone 4 only to add Redis as a non-authoritative, version-checked cache and coordination accelerator, including real Redis TTL/invalidation/outage/lease tests and explicit PostgreSQL fallback.
...
Milestones 0, 1, 2, and 3 are complete. The first unchecked milestone is Milestone 4. Execute only Redis hot-state cache and Worker coordination with PostgreSQL remaining authoritative; do not begin candidate governance or install Mem0/pgvector dependencies early, and stop before Milestone 5.
