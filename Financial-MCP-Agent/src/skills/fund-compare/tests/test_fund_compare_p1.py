...
def test_fund_compare_route_hits_financial_sop() -> None:
    ...
    assert route.selected_skill == "financial-sop"
    assert route.selected_skill_family == "financial-sop"
    assert route.skill_name == "fund-compare"
...
def test_fund_compare_follow_up_reuses_previous_context() -> None:
    route = asyncio.run(
        route_chat_skill(
            "请重新回答",
            conversation_context="recent_messages:\n用户: 请对比华安黄金ETF和博时黄金ETF，按收益、规模、费率和流动性给我一个结构化结论",
        )
    )
    assert route.selected_skill == "financial-sop"
    assert route.skill_name == "fund-compare"
    assert route.arguments.get("is_follow_up") is True
