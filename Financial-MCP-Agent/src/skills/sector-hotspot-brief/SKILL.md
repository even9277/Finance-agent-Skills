...
为板块、行业、主题或概念做热点简报。这个 skill 先确认板块层面事实，再补代表性成分和指数上下文，用于回答“最近强不强、谁在带、还能不能追、风险在哪里”这类问题。
...
- 用户只问单只股票 first pass，应交给 `stock-first-pass`。
- 用户问板块或成分为什么异动且需要新闻催化，应交给 `market-move-explain`。
- 用户是在筛选 ETF，不应把板块简报当成 ETF 推荐。
- 板块主语完全无法识别时，应先追问。
...
- `get_sector_snapshot` 是板块整体事实主工具，优先执行。
- `get_sector_constituents` 用于确认代表性成分、龙头候选和内部扩散程度。
- `get_index_bars` 用于补充指数或行业基准上下文，不能替代板块快照。
- 不调用网页/新闻搜索；如需要解释事件驱动，应转给 `market-move-explain`。

## Evidence Rules

- `sector_snapshot` 是首选主证据；如果缺失，只能用 `index_daily` 做降级上下文。
- 龙头判断必须依赖成分股或板块内部证据，不能只凭模型常识。
- “还能不能追”必须带追高、拥挤、回撤和主题轮动风险。
- 如果只有指数没有板块数据，要明确“当前按指数口径近似观察”。
...
- `references/板块简报口径.md`：板块与指数口径、龙头判断和追高风险表达。
