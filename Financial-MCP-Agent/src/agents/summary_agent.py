...
def _should_truncate_report() -> bool:
    """
    是否启用“按基准时间行截断”。

    默认关闭：过去的逻辑会在匹配到任意包含 current_time_info 的行时截断，
    极易把完整报告误截断成只有开头一小段（线上已观测到长度=1584的截断报告）。
    仅在明确需要兼容某些模型输出“追加冗余尾巴”的场景下，通过环境变量开启。
    """
...
def _build_memory_context_prompt(memory_context: dict) -> str:
    """
    Phase 3 LTM：将 memory_context 构建为 summary_agent 的个性化注入段。

    只在 ENABLE_MEMORY=true 且 memory_context 有实质内容时注入。
    全部为 None/空 时不注入（避免注入无意义的空画像）。

    硬约束：
    1. 关键结论必须以实时分析数据为准，画像仅供参考
    2. 若用户偏好与标的风险水平不匹配，必须在报告中显式标注冲突
    3. 画像中为 None 的字段，不得编造
    """
    enable_memory = os.getenv("ENABLE_MEMORY", "false").lower() in ("true", "1", "yes")
    if not enable_memory or not memory_context:
        return ""
    ...
    if risk_level:
        ...
        parts.append(f"风险偏好：{risk_display}")
    ...
    if horizon:
        ...
        parts.append(f"持有周期：{horizon_display}")

    ret_min = profile.get("expected_return_min")
    ret_max = profile.get("expected_return_max")
    if ret_min is not None or ret_max is not None:
        if ret_min is not None and ret_max is not None:
            parts.append(f"期望收益：{ret_min}% ~ {ret_max}%")
        elif ret_min is not None:
            parts.append(f"期望收益：≥ {ret_min}%")
        else:
            parts.append(f"期望收益：≤ {ret_max}%")

    sectors = profile.get("sectors", [])
    if sectors:
        parts.append(f"关注板块：{', '.join(sectors)}")

    constraints = profile.get("constraints", [])
    if constraints:
        parts.append(f"约束条件：{', '.join(constraints)}")

    response_pref = profile.get("response_pref")
    if response_pref and response_pref != "balanced":
        pref_display = {
            "concise": "简洁", "detailed": "详细", "risk_first": "先讲风险",
        }.get(response_pref, response_pref)
        parts.append(f"回答偏好：{pref_display}")

    if semantic_memories:
        semantic_texts = [m.get("text", "") for m in semantic_memories if m.get("text")]
        if semantic_texts:
            parts.append(f"补充历史偏好线索（置信度低于上方结构化字段）：")
            for t in semantic_texts[:3]:  # 最多3条
                parts.append(f"  - {t}")
    ...
    parts.append(
        ...
        "2. 若用户关注板块与当前标的所属板块一致，在报告中明确指出该关联性；\n"
        "3. 若用户风险偏好与标的风险水平不匹配，必须在「风险提示」章节显式标注该冲突，不得迎合偏好；\n"
    ...
    result = "\n".join(parts)
    logger.info(f"[LTM] 注入 memory_context 到 summary_agent，长度={len(result)}")
    print(f"[LTM] 注入用户投资画像到 summary_agent（{len(parts)-2} 个字段）")
    return result
...
async def summary_agent(state: AgentState) -> Dict[str, Any]:
    ...
    try:
        ...
        **重要提醒：**
        ...
        # Phase 3 LTM：若 memory_context 存在且非空，拼装个性化画像注入段
        memory_context = current_data.get("memory_context", {})
        memory_prompt_segment = _build_memory_context_prompt(memory_context)

        # Phase 2 STM：若 prepare_summary_context 已拼装上下文，则优先使用
        summary_input_context = current_data.get("summary_input_context", "")

        if summary_input_context.strip():
            # STM 模式：使用 prepare_summary_context 拼装的三段式上下文
            logger.info(f"{WAIT_ICON} SummaryAgent: 使用 STM summary_input_context（长度={len(summary_input_context)}）")
            user_prompt = f"""
        ...
        CONVERSATION CONTEXT (STM - short-term memory, for reference only):
        ...
        """
            ...
            # 原始模式（ENABLE_STM=false）：直接使用各 analyst 分析结果
            user_prompt = f"""
        Please create a comprehensive analysis report for {company_name} ({stock_code}) based on the following analyses.
        ...
        # 默认不截断报告：避免误把完整报告截断到时间信息行
        if _should_truncate_report():
