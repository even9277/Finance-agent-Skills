...
请使用可用的工具获取实际数据进行分析，而不是基于假设。如果某些数据无法获取，请尝试使用不同的时间周期或其他工具组合，基于可用信息提供尽可能全面的分析。"""
            ...
            safe_error = "Fundamental analysis failed."
            logger.error(
                "%s FundamentalAgent: MCP or agent execution failed; error_type=%s",
                ERROR_ICON,
                type(exc).__name__,
            )
            current_data["fundamental_analysis_error"] = safe_error
            current_data["fundamental_analysis"] = "基本面分析暂时不可用。"
            current_metadata["fundamental_agent_error_type"] = type(exc).__name__
        ...
        safe_error = "Fundamental analysis failed."
        logger.error(
            "%s FundamentalAgent: execution failed; error_type=%s",
            ERROR_ICON,
            type(exc).__name__,
        )
        current_data["fundamental_analysis_error"] = safe_error
